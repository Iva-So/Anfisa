...
def what_conclusion(parsed_temperature):
    try:
        ...
        if temperature < 18:
            return 'Холодно для мороженого! Лучше идти под одеяло'
        elif 18 <= temperature <= 27:
            return 'В самый раз погодка, угощайся!'
        else:
            return 'Жарко, как в Ташкенте! Бери двойную порцию!' # Если (if) температура строго меньше 18:
